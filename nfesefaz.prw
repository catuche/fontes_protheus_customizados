	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If ((cAliasSD2)->D2_TIPO == "D" .and. !lIpiDev)  .Or. lConsig .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM ) .OR. ((cAliasSD2)->D2_TIPO == "B" .and. lIpiBenef) .OR. ((cAliasSD2)->D2_TIPO=="P" .And. lComplDev .And. !lIpiDev)
							aTotal[01] += (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3 + iif(cVerAmb >= "4.00",0,(cAliasSD2)->D2_VALIPI) + nIcmsST
						Else 
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_DESPESA + (cAliasSD1)->D1_VALPS3 + (cAliasSD2)->D2_VALCF3 + nIcmsST + IIF( ((cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev) .Or. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef),iif(cVerAmb >= "4.00",0,(cAliasSD1)->D1_VALIPI),0),;//Outras despesas + PISST + COFINSST  (Inclusão do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).
					nRedBC,;//% Redução da Base de Cálculo
					cCST,;//Cód. Situação Tributária
				...
				If ((cAliasSD1)->D1_TIPO == "D" .and. !lIpiDev .and. cTipo == "1")  .Or. ((cAliasSD1)->D1_TIPO == "D" .and. !lEipiDev ) .Or. lConsig .Or. (Alltrim((cAliasSD1)->D1_CF) $ cMVCFOPREM ) .OR. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef) .OR. ((cAliasSD1)->D1_TIPO=="P" .And. lComplDev .And. !lIpiDev)
					aTotal[01] += (cAliasSD1)->D1_DESPESA + (cAliasSD1)->D1_VALPS3 + (cAliasSD1)->D1_VALCF3 + iif(cVerAmb >= "4.00",0,(cAliasSD1)->D1_VALIPI) + nIcmsST
				Else 
...
IF cVeramb >= "4.00"
   //Ajute para alimetar o aDetPag
	...
	ElseIf Len(aDupl) == 0
		cForma:= '99'  //outros
	ElseIf Len(aDupl) > 0	
   		cForma :="14"  //14=Duplicata Mercantil  //15 crtao   
	Endif
	aadd(aDetPag,{cForma,aTotal[2],0.00})      
EndIf
                   
IF lPe01Nfe     

	//FIABESA: Alterado para enviar os arrays de impostos
	aParam := {aProd,;
			   cMensCli,;
			   cMensFis,;
			   aDest,;
...
cString += '<eantrib>'+IIF(!Empty(aProd[45]),ConvType(aProd[45]), iif( aProd[08] <> aProd[11],"", ConvType(aProd[03])))+'</eantrib>'
